...
static bool slcan_can_msg_from_buf_t(slcan_can_msg_t* can_msg, slcan_can_msg_extdata_t* ed, slcan_cmd_buf_t* buf)
{
    assert(can_msg != NULL);
    assert(buf != NULL);

    size_t buf_data_size = slcan_cmd_buf_size(buf);
    if(buf_data_size < 1 + 3 + 1 + 1 /* EOM */) return false;
    ...
    // check data size.
    //for(i = 0; i < 3 + 1; i ++){
        if(!isxdigit(cmd_data[0])) return false;
    //}
    ...
    if(buf_data_size < msg_size) return false;
    ...
    // last byte must be '\r'.
    if(cmd_data[0] != SLCAN_CMD_OK) return false;
...
static bool slcan_can_msg_from_buf_T(slcan_can_msg_t* can_msg, slcan_can_msg_extdata_t* ed, slcan_cmd_buf_t* buf)
{
    ...
    size_t buf_data_size = slcan_cmd_buf_size(buf);
    if(buf_data_size < 1 + 8 + 1 + 1 /* EOM */) return false;
...
static bool slcan_can_msg_from_buf_r(slcan_can_msg_t* can_msg, slcan_can_msg_extdata_t* ed, slcan_cmd_buf_t* buf)
{
    assert(can_msg != NULL);
    assert(buf != NULL);
...
bool slcan_can_msg_from_buf(slcan_can_msg_t* can_msg, slcan_can_msg_extdata_t* ed, slcan_cmd_buf_t* buf)
{
    if(can_msg == NULL || buf == NULL) return false;
...
static bool slcan_can_msg_to_buf_t(slcan_can_msg_t* can_msg, slcan_can_msg_extdata_t* ed, slcan_cmd_buf_t* buf)
{
    assert(can_msg != NULL);
    assert(buf != NULL);
    ...
    if(slcan_cmd_buf_put(buf, SLCAN_CMD_TRANSMIT) == 0) return false;

    // id.
    uint32_t id = can_msg->id;
    if(slcan_cmd_buf_put(buf, digit_num_to_hex((id >> 8) & 0x0f)) == 0) return false;
    if(slcan_cmd_buf_put(buf, digit_num_to_hex((id >> 4) & 0x0f)) == 0) return false;
    if(slcan_cmd_buf_put(buf, digit_num_to_hex((id >> 0) & 0x0f)) == 0) return false;

    // data_size.
    if(slcan_cmd_buf_put(buf, digit_num_to_hex(can_msg->data_size)) == 0) return false;

    int i;
    // data.
    for(i = 0; i < can_msg->data_size; i ++){
        if(slcan_cmd_buf_put(buf, digit_num_to_hex((can_msg->data[i] >> 4) & 0x0f)) == 0) return false;
        if(slcan_cmd_buf_put(buf, digit_num_to_hex((can_msg->data[i] >> 0) & 0x0f)) == 0) return false;
    }

    if(ed){
        if(ed->has_timestamp){
            uint16_t ts_value = ed->timestamp;
            if(slcan_cmd_buf_put(buf, digit_num_to_hex((ts_value >> 12) & 0x0f)) == 0) return false;
            if(slcan_cmd_buf_put(buf, digit_num_to_hex((ts_value >> 8) & 0x0f)) == 0) return false;
            if(slcan_cmd_buf_put(buf, digit_num_to_hex((ts_value >> 4) & 0x0f)) == 0) return false;
            if(slcan_cmd_buf_put(buf, digit_num_to_hex((ts_value >> 0) & 0x0f)) == 0) return false;
        }
        if(ed->autopoll_flag){
...
static bool slcan_can_msg_to_buf_T(slcan_can_msg_t* can_msg, slcan_can_msg_extdata_t* ed, slcan_cmd_buf_t* buf)
{
    ...
    if(slcan_cmd_buf_put(buf, SLCAN_CMD_TRANSMIT_EXT) == 0) return false;

    // id.
    uint32_t id = can_msg->id;
    if(slcan_cmd_buf_put(buf, digit_num_to_hex((id >> 28) & 0x0f)) == 0) return false;
    if(slcan_cmd_buf_put(buf, digit_num_to_hex((id >> 24) & 0x0f)) == 0) return false;
    if(slcan_cmd_buf_put(buf, digit_num_to_hex((id >> 20) & 0x0f)) == 0) return false;
    if(slcan_cmd_buf_put(buf, digit_num_to_hex((id >> 16) & 0x0f)) == 0) return false;
    if(slcan_cmd_buf_put(buf, digit_num_to_hex((id >> 12) & 0x0f)) == 0) return false;
    if(slcan_cmd_buf_put(buf, digit_num_to_hex((id >> 8) & 0x0f)) == 0) return false;
    if(slcan_cmd_buf_put(buf, digit_num_to_hex((id >> 4) & 0x0f)) == 0) return false;
    if(slcan_cmd_buf_put(buf, digit_num_to_hex((id >> 0) & 0x0f)) == 0) return false;
...
static bool slcan_can_msg_to_buf_r(slcan_can_msg_t* can_msg, slcan_can_msg_extdata_t* ed, slcan_cmd_buf_t* buf)
{
    assert(can_msg != NULL);
    assert(buf != NULL);
    ...
    if(slcan_cmd_buf_put(buf, SLCAN_CMD_TRANSMIT_RTR) == 0) return false;
...
static bool slcan_can_msg_to_buf_R(slcan_can_msg_t* can_msg, slcan_can_msg_extdata_t* ed, slcan_cmd_buf_t* buf)
{
    ...
    if(slcan_cmd_buf_put(buf, SLCAN_CMD_TRANSMIT_RTR_EXT) == 0) return false;
...
bool slcan_can_msg_to_buf(slcan_can_msg_t* can_msg, slcan_can_msg_extdata_t* ed, slcan_cmd_buf_t* cmd)
{
